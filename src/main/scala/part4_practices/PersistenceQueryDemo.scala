...
object PersistenceQueryDemo extends App {
  ...
  val simpleActor = system.actorOf(Props[SimplePersistentActor], "simplePersistent")
  ...
  val events = readJournal.eventsByPersistenceId("persistence-quiry-id-1", 0, Long.MaxValue)
  ...
  class MusicStoreEventAdapter extends WriteEventAdapter {
    ...
    override def toJournal(event: Any): Any = event match {
      case PlaylistPurchased(_, songs) =>
