...
import part4_practices.DetachingModels.DomainModel.{ApplyCoupon, Coupon, CouponApplied, User}
...
object DetachingModels extends App {
  ...
  class ModelAdapter extends EventAdapter {
    import part4_practices.DetachingModels.DomainModel._
    import part4_practices.DetachingModels.DataModel._

    override def manifest(event: Any): String = "CMA"

    override def fromJournal(event: Any, manifest: String): EventSeq = event match {
      case event @ WrittenCouponApplied(code, userId, userEmail) =>
        println(s"Coverting $event to Domain model")
