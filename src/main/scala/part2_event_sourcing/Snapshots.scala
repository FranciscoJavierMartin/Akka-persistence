...
object Snapshots extends App {
  ...
  class Chat(owner: String, contact: String) extends PersistentActor with ActorLogging {
    ...
    override def receiveRecover: Receive = {
      case ReceivedMessageRecord(id,contents) =>
        log.info(s"Recovered received message: $contents")
        maybeReplaceMessage(contact,contents)
        currentMessageId = id
  ...
  val chat = system.actorOf(Chat.props("Michael", "John"))
...
//  }
  ...
  /*
    pattern:
      ...
      - (optional, but best practice) handle SaveSnapshotSuccess ans SaveSnapshotFailure in receiveCommand
